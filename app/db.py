...
DB_PATH = os.getenv("JML_DB_PATH", os.path.join(os.path.dirname(__file__), "jml.db"))
...
def log_hr_event(event_type: str, employee_id: str, payload_json: str) -> None:
    ...
    with db_conn() as conn:
        conn.execute(
            """
            INSERT INTO hr_events (ts, event_type, employee_id, payload_json)
            VALUES (?, ?, ?, ?);
            """,
...
def list_events(limit: int = 100) -> List[Dict[str, Any]]:
    with db_conn() as conn:
        rows = conn.execute(
            """
            SELECT id,ts,event_type,employee_id,payload_json
            FROM hr_events
            ORDER BY id DESC
            LIMIT ?;
            """,
